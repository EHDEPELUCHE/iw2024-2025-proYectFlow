...
@Uses(Icon.class)
@AnonymousAllowed
public class ProyectosEnDesarrolloView extends Div {
    private final ProyectoService proyectoService;
    private Grid<Proyecto> grid;
    private Filters filters;

    public ProyectosEnDesarrolloView(ProyectoService proyectoService, AuthenticatedUser user) {
        H1 h1Titulo = new H1("Proyectos que se están desarrollando actualmente");
        ...
        if (GLOBALES.FECHA_LIMITE.compareTo(hoy) > 0) {
            filters = new Filters() {
                ...
                public Predicate toPredicate(Root<Proyecto> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
                    Predicate estadoPredicate = criteriaBuilder.equal(root.get("estado"), Proyecto.Estado.enDesarrollo);
    ...
    public static abstract class Filters extends Div implements Specification<Proyecto> {
        private final Proyecto.Estado estado = Proyecto.Estado.enDesarrollo;

...
public class ProyectosPromotorView extends Div {
    ...
    public ProyectosPromotorView(ProyectoService proyectoService, AuthenticatedUser user) {
        ...
        filters = new Filters() {
            ...
            public Predicate toPredicate(Root<Proyecto> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
                Predicate promotorPredicate = criteriaBuilder.equal(root.get("promotor"), user.get().orElse(null));
                Predicate estadoPredicate = criteriaBuilder.equal(root.get("estado"), Proyecto.Estado.solicitado);
                return criteriaBuilder.and(promotorPredicate, estadoPredicate);
            }
        };
